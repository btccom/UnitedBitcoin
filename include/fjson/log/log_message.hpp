...
namespace fjson
{
   ...
   class log_context 
   {
      public:
        ...
        void          append_context( const fjson::string& c );
   ...
   class log_message
   {
      public:
         log_message();
         /**
          *  @param ctx - generally provided using the FJSON_LOG_CONTEXT(LEVEL) macro 
          */
...
FJSON_REFLECT_TYPENAME(fjson::log_message );
...
/**
 * @def FJSON_LOG_CONTEXT(LOG_LEVEL)
 * @brief Automatically captures the File, Line, and Method names and passes them to
 *        the constructor of fjson::log_context along with LOG_LEVEL
 * @param LOG_LEVEL - a valid log_level::Enum name.
 */
#define FJSON_LOG_CONTEXT(LOG_LEVEL) \
   fjson::log_context( fjson::log_level::LOG_LEVEL, __FILE__, __LINE__, __func__ )
   
/**
 * @def FJSON_LOG_MESSAGE(LOG_LEVEL,FORMAT,...)
 *
 ...
 */
#define FJSON_LOG_MESSAGE( LOG_LEVEL, FORMAT, ... ) \
   fjson::log_message( FJSON_LOG_CONTEXT(LOG_LEVEL), FORMAT, fjson::mutable_variant_object()__VA_ARGS__ )
