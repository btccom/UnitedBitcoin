...
int GetJmpAddr(Function* F, int addr) {
	...
	while (GET_OPCODE(F->f->code[real_end]) == UOP_JMP) {
...
void PrintLogicItem(std::stringstream &str, LogicExp* exp, int inv, int rev) {
	if (exp->subexp) {
		...
		if ((exp->op != UOP_TEST) && (exp->op != UOP_TESTSET)) {
	...
	*thenaddr = last->pc + 2;
	...
	for (curr = realLast; curr; curr = lua_cast(BoolOp*, curr->super.prev)) {
		...
		if ((elseaddr > *thenaddr) &&
			(((curr->op == UOP_TEST) || (curr->op == UOP_TESTSET)) ? (dest > elseaddr + 1) :
			(dest > elseaddr))) {
			last = curr;
			*thenaddr = curr->pc + 2;
			elseaddr = dest;
	...
	for (curr = lua_cast(BoolOp*, first->super.next), lastCount = 1; curr && curr != lua_cast(BoolOp*, last->super.next); curr = lua_cast(BoolOp*, curr->super.next), lastCount++) {
		...
		if (dest < firstaddr) {
			...
			currExp = exp;
			if (endif) {
				if ((op->op != UOP_TEST) && (op->op != UOP_TESTSET)) {
	...
	*/
	...
	for (i = startparams; i < F->f->sizelocvars; i++) {
		if (F->f->locvars[i].startpc == F->pc) {
			...
			// handle FOR loops
			if (GET_OPCODE(instr) == UOP_FORPREP) {
				F->f->locvars[i].startpc = F->pc + 1;
				continue;
			}
			// handle TFOR loops
			if (GET_OPCODE(instr) == UOP_JMP) {
...
int CompareProto(const Proto* fleft, const Proto* fright, std::stringstream &str) {
	...
	for (pc = 0; pc < minsizecode; pc++) {
		...
		if (ileft == iright) {
			...
			if (left.op == UOP_EQ && right.op == UOP_EQ) {
				if (left.a == right.a && left.b == right.c && left.c == right.b) {
					sizesame++;
				}
			}
			else if ((left.op == UOP_LT && right.op == UOP_LE) ||
				(left.op == UOP_LE && right.op == UOP_LT)) {
				if (left.a == !right.a && left.b == right.c && left.c == right.b) {
					sizesame++;
				}
			}
...
int isTestOpCode(OpCode op) {
	return (op == UOP_EQ || op == UOP_LE || op == UOP_LT || op == UOP_TEST || op == UOP_TESTSET);
}
...
std::string ProcessCode(GluaDecompileContextP ctx, Proto* f, int indent, int func_checking, std::string funcnumstr)
{
	...
	*/
	...
	for (pc = n - 1; pc >= 0; pc--) {
		...
		Instruction i_1 = 0;
		OpCode o_1 = (OpCode) (UOP_VARARG + 1);
		if (pc > 0) {
			i_1 = code[pc - 1];
			o_1 = GET_OPCODE(i_1);
		}
		...
		if (pc > 0 && o_1 == UOP_SETLIST && GETARG_C(i_1) == 0) {
			continue;
		}
		if (o == UOP_SETLIST && GETARG_C(i) == 0) {
			continue;
		}
		...
		real_end = GetJmpAddr(F, pc + 1);
			...
			if (o == UOP_JMP && a > 0) {
				// instead UOP_CLOSE in 5.2 : if (A) close all upvalues >= R(A-1)
				int startreg = a - 1;

				AddToSet(F->do_opens, f->locvars[startreg].startpc);
				AddToSet(F->do_closes, f->locvars[startreg].endpc);
				// continue; // comment out because of 5.2 UOP_JMP
			}
			...
			for (pc = 0; pc < n; pc++) {
				...
				*/
				...
				switch (o) {
				case UOP_MOVE:
					/* Upvalue handling added to UOP_CLOSURE */
				{
					std::string bstr;
					if (a == b)
						break;
				...
				case UOP_VARARG: // Lua5.1 specific.
				{
					int i;
					/*
					* Read ... into register.
				...
				case UOP_JMP:
				{
					...
					if (foundInt != nullptr) { // break
					...
					else if (sbc == 2 && GET_OPCODE(code[pc + 2]) == UOP_LOADBOOL) {
						/*
						...
						* ::jmp_target
						*/
						fprintf(stderr, "processing UOP_JMP to } else if (sbc == 2 && GET_OPCODE(code[pc+2]) == UOP_LOADBOOL) { \n");
						fprintf(stderr, " at line %d in file %s\n", __LINE__, __FILE__);
						fprintf(stderr, " for lua files: ");
						printFileNames(stderr);
						fprintf(stderr, "\n");
						...
						fflush(stderr);
						{
							...
							AddToList(&(F->bools), (ListItem*)MakeBoolOp(ra, ra, UOP_TESTSET, c, pc + 3, dest));
							F->testpending = a + 1;
							F->testjump = dest;
							TRY(ctx, test = OutputBoolean(ctx, F, &thenaddr, nullptr, 1));
							...
							ignoreNext = 2;
					...
					else if (GET_OPCODE(idest) == UOP_LOADBOOL) { // WHY
																 /*
																 ...
																 * LOADBOOL
																 */
						fprintf(stderr, "processing UOP_JMP to } else if (GET_OPCODE(idest) == UOP_LOADBOOL) { \n");
						fprintf(stderr, " at line %d in file %s\n", __LINE__, __FILE__);
						fprintf(stderr, " for lua files: ");
						printFileNames(stderr);
						fprintf(stderr, "\n");
						...
						fprintf(stderr, "processing UOP_JMP to } else { \n");
						fprintf(stderr, " at line %d in file %s\n", __LINE__, __FILE__);
						fprintf(stderr, " for lua files: ");
						printFileNames(stderr);
						fprintf(stderr, "\n");
						...
						fflush(stderr);
						{
							...
							for (;;) {
								...
								if (GET_OPCODE(nextins) == UOP_JMP && GETARG_sBx(nextins) == nextsbc) {
									nextpc++;
									nextsbc--;
								}
								else
			...
			LOGIC_NEXT_JMP:
				...
				case UOP_TAILCALL:
				{
					...
					*/
					...
					if (GET_OPCODE(code[pc + 1]) == UOP_LOADNIL && GETARG_A(code[pc + 1]) == a + 1) {
				...
				case UOP_FORLOOP: //Lua5.1 specific. TODO: CHECK
				{
					AstStatement* currStmt = F->currStmt;
					...
					assert(GET_OPCODE(i_forprep) == UOP_FORPREP);
					r_begin = a_forprep;
					r_end = a_forprep + 3;
				...
				{
					// 5.1 UOP_TFORLOOP	A C		R(A+3), ... ,R(A+2+C) := R(A)(R(A+1), R(A+2));
					//	if R(A+3) ~= nil then R(A+2)=R(A+3) else pc++
					// 5.1 UOP_JMP sBx	pc += sBx

					// 5.2 UOP_TFORCALL	A C		R(A+3), ... ,R(A+2+C) := R(A)(R(A+1), R(A+2));
					// 5.2 UOP_TFORLOOP	A sBx	if R(A+1) ~= nil then { R(A)=R(A+1); pc += sBx }
				...
				case UOP_TFORLOOP:
					break;
				case UOP_FORPREP: //Lua5.1 specific. TODO: CHECK
				{
					/*
					* numeric 'for'
					*/
					...
					if (!IS_VARIABLE(a + 3)) {
						...
						for (i = 0; i < f->sizelocvars; i++) {
							if (f->locvars[i].startpc == pc + 1) {
								loopvars++;
								//search for the loop variable. Set it's endpc one step further so it will be the same for all loop variables
								if (GET_OPCODE(F->f->code[f->locvars[i].endpc - 1]) == UOP_FORLOOP) {
									f->locvars[i].endpc -= 1;
								}
								if (loopvars == 4) {
									idxname = LOCAL(ctx, i);
				...
				case UOP_SETLIST:
				{
					if (c == 0) {
						...
						ignoreNext = 1;
						if (GET_OPCODE(i_next_arg) == UOP_EXTRAARG) {
