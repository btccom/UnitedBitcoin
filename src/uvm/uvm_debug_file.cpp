...
namespace uvm
{
	namespace lua
	{
		namespace core
		{
			...
			namespace gle = uvm::lua::exceptions;
			...
			LuaDebugFileInfo LuaDebugFileInfo::deserialize(const std::string& content)
			{
				...
				for (auto it = pt_of_source_line_mappings.begin(); it != pt_of_source_line_mappings.end(); ++it) {
					size_t uvm_line = std::stoi(it->first);
					size_t lua_line =std::stoi(it->second.data());
					ldf._source_line_mappings[uvm_line] = lua_line;
				}
			...
			void LuaDebugFileInfo::set_source_line_mapping(size_t uvm_line, size_t lua_line)
			{
				_source_line_mappings[uvm_line] = lua_line;
			}
			...
			size_t LuaDebugFileInfo::find_uvm_line_by_lua_line(size_t lua_line)
			{
				for(const auto &item : _source_line_mappings)
				{
			...
			size_t LuaDebugFileInfo::find_lua_line_by_uvm_line(size_t uvm_line)
			{
				for(const auto &item : _source_line_mappings)
				{
					if (item.first >= uvm_line)
						return item.second;
				}
				return 0;
			}
