...
static const char *progname = "uvm_lua";
...
void lua_fill_contract_info_for_use(lua_State *L)
{
    ...
    lua_setmetatable(L, -2); // {}, uvm, {}   设置storage的metatable为storage_mt
    lua_pop(L, 3);
}
...
int luaL_import_contract_module_from_address(lua_State *L)
{
    ...
    if (!lua_isnil(L, -1))  /* non-nil return? */
    {
        if (lua_istable(L, -1)) {
            ...
            char error_msg[LUA_COMPILE_ERROR_MAX_LENGTH] = "\0";
            /*
            snprintf(error_msg, LUA_COMPILE_ERROR_MAX_LENGTH - 1, "this uvm contract not return a table");
            if (strlen(L->compile_error) < 1)
            memcpy(L->compile_error, error_msg, LUA_COMPILE_ERROR_MAX_LENGTH);
            global_uvm_chain_api->throw_exception(L, UVM_API_SIMPLE_ERROR, error_msg);
            */
	...
	struct LuaCompileFilePreloadResult *result, bool use_type_check = false, GluaModuleByteStreamP stream = nullptr, bool is_contract = false)
{
	...
	auto code = uvm::lua::lib::pre_modify_lua_source_code(L, filename, error, nullptr, use_type_check, stream, is_contract);
	if (global_uvm_chain_api->has_exception(L))
    {
    ...
    if (status != 0)
    {
		...
		if (lf.f) fclose(lf.f);
        const char *error_str = "parse uvm lua code error";
        if (strlen(L->compile_error) > 0)
            error_str = L->compile_error;
        if (error)
            strncpy(error, error_str, LUA_COMPILE_ERROR_MAX_LENGTH);
        global_uvm_chain_api->throw_exception(L, UVM_API_PARSER_ERROR, error_str);
        // lcompile_error_set(L, error, "parse uvm lua code error");
        return LUA_ERRFILE;
    }
    readstatus = ferror(lf.f);
