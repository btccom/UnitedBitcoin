...
namespace uvm {
    namespace parser {
		...
		void GluaTypeChecker::add_emit_event_type(std::string event_type_name)
		{
			...
			{
				if (!uvm::util::vector_contains(_emit_events, event_type_name))
        ...
        void GluaTypeChecker::generate_record_constructor_code(MatchResult *mr, GluaTypeInfoP record, MatchResult *parent_mr)
        {
            ...
            if (record->is_record())
            {
                ...
                {
                    ss << "        props['" << item.first << "'] = props['" << item.first << "'] or " << uvm::util::string_trim(item.second) << "\n";
                }
                ss << "    end\n";
				...
				auto lines_added = uvm::util::string_lines_count(ss.str())-1;
		...
		bool GluaTypeChecker::check_if_stat_error(uvm::parser::GluaIfStatNode *ast_node, GluaTypeInfoP result_type, GluaTypeInfoP ret_type)
		{
			auto parent_tree = current_checking_proto();
				...
				if (mr->is_complex())
				{
						...
						|| prop_mr->as_final()->token.type == TOKEN_RESERVED::LTK_NAME))
					{
						...
						if (obj_type_info->is_contract_type() && is_checking_contract)
						{
							...
							{
								if (uvm::util::vector_contains(uvm::lua::lib::contract_special_api_names, prop_name)
									|| prop_name == "storage")
								{
							...
							if (cur_namelist_mr->is_complex() && cur_namelist_mr->as_complex()->size() > 0)
							{
								...
								if (name_mr->node_name() == "name_in_record_with_type_declare" && name_mr->as_complex()->size() >= 5)
								{
									...
									else
									{
										...
										default_value_code = uvm::util::string_trim(token_parser->dump());
            ...
            GluaTypeInfoP result_typ, GluaTypeInfoP ret_type, MatchResult *parent_mr)
        {
			...
			{
				auto *ast_node = (uvm::parser::GluaAstNode*) mr;
				switch(ast_node->ast_node_type())
				{
				case GluaAstNodeType::LA_IF_STAT:
					return check_if_stat_error((uvm::parser::GluaIfStatNode*) ast_node, result_typ, ret_type);
					break;
				default:
        ...
        {
            typestr = uvm::util::string_trim(typestr);
			...
			else if(uvm::util::starts_with(typestr, "("))
			{
				...
				for (auto i = arg_type_strs.begin(); i != arg_type_strs.end(); ++i)
				{
					...
					arg_type_str = uvm::util::string_trim(arg_type_str);
			...
			else if(uvm::util::starts_with(typestr, "record"))
			{
				...
				auto props_str_list = uvm::util::string_split(props_all_str, ';');
				for(const auto &prop_pair_str : props_str_list)
				{
					...
					if (prop_name.length() < 1)
						continue;
					prop_name = uvm::util::string_trim(prop_name);
					auto prop_type_str = prop_pair_str.substr(prop_pair_str.find_first_of(":") + 1);
					prop_type_str = uvm::util::string_trim(prop_type_str);
            ...
            else
            {
                ...
                if (left_xkh_idx > 0 && left_xkh_idx < typestr.length() - 1)
                {
                    ...
                    auto ret_type_info = of_type_str(ret_type_str, extra);
                    auto arg_type_strs = uvm::util::string_split(args_type_str, ',');
                    type_info->ret_types.push_back(ret_type_info);
                    for (auto i = arg_type_strs.begin(); i != arg_type_strs.end(); ++i)
                    {
        ...
        {
            auto *global_var_infos = uvm::lua::lib::get_globalvar_type_infos();
		...
		bool GluaTypeChecker::check_contract_syntax_tree_type(MatchResult *mr, GluaTypeInfoP *ret_contract_type, GluaTypeInfoP *ret_contract_storage_type_out)
        {
			...
			for(const auto &api_type : ret_type->record_props)
			{
				...
				else if(uvm::util::vector_contains(uvm::lua::lib::contract_special_api_names, api_prop_name)
					&& !uvm::util::vector_contains(uvm::lua::lib::contract_int_argument_special_api_names, api_prop_name))
				{
				...
				else if(uvm::util::vector_contains(uvm::lua::lib::contract_int_argument_special_api_names, api_prop_name))
				{
        ...
        bool GluaTypeChecker::check_syntax_tree_type(MatchResult *mr, GluaTypeInfoP ret_type, bool is_contract)
        {
			...
			ldf = std::make_shared<uvm::lua::core::LuaDebugFileInfo>();
                    ...
                    if (cmr->size() >= 2)
                    {
                        ...
                        if (item2->node_name() == "args" || item2->node_name() == "tableconstructor")
                        {
                            ...
                            if (is_function)
                            {
								...
								if (item1->is_final())
								{
									if(uvm::util::vector_contains(exclude_function_names, funcname_token.token))
									{
						...
						else if(item2->is_final() && item2->as_final()->token.token == ":" && cmr->size()==4)
						{
							...
							{
								if(uvm::util::vector_contains(uvm::lua::lib::contract_special_api_names, funcname_token.token))
								{
                ...
                else if (mr->is_complex())
                {
                    ...
                    for (auto i = cmr->items.begin(); i != cmr->items.end(); ++i)
                    {
                        ...
                        if (item->hidden())
                        {
							...
							{
								auto replacement_token = literal_code_token(uvm::util::string_trim(item->hidden_replace_string()));
