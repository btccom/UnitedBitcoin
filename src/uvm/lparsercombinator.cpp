...
namespace uvm {
    namespace parser {
		...
		ParseResult GluaTypeParserFunctor::tryParse(Input &input, size_t skip_end_size2, bool optional)
        {
			...
			// TODO: ctx record_nameparserfunctor，parse
			return _ctx->failure_parser()->tryParse(input, 0, optional);
        }
        ...
        ParseResult ConcatParserFunctor::tryParse(Input &input, size_t skip_end_size2, bool optional)
        {
            ...
            if (parser_min_length > 0 && !input.can_read(parser_min_length, skip_end_size2))
                return _ctx->failure_parser()->tryParse(input, 0, optional);
			auto it = 0;
			auto start_match_pos = input.pos(); // token
			ParserFunctor *cur_parser = _parsers[it];
			// TODO: optionaltrue，expect，errorfailure
            // TODO: _parserstokens， exp ::= exp binop exp ，exp，，
            // TODO: ，，options
            ...
            if (pr.state() == ParserState::SUCCESS)
            {
                ...
                while (it < _parsers.size())
                {
					...
					if(!pr.result()->next_input().can_read(1, skip_end_size + skip_end_size2))
					{
						...
						{
							// _parsers（literal，->str()）maybe error
                    ...
                    else if (pr2.state() == ParserState::FAILURE)
                    {
						...
						// ，，maybe_errors
						// concat_parserliteral，union parser，literalunion parser，maybe error
						// concat_parserliteral，union parser，literalunion parserparser，maybe error()
                        if(node_name().length()>0 && _parsers.size()>0 && _parsers[0]->is_literal() && parent_parser() && parent_parser()->is_union())
                        {
							auto head_literal_name = _parsers[0]->str(); // literal parser
							...
							if (index_in_parent >= 0)
							{
								...
								// TODO: parser，literal，maybe error
        ...
        std::string ParserContext::simple_token_error(GluaParserToken token, bool with_maybe_errors) const
        {
            ...
            ss << "error not match after token " << token.token
                << ", line " << (token.linenumber-7+2) // FIXME： 
        ...
        size_t ParserContext::parser_min_length(ParserFunctor *parser)
        {
            ...
            set_parser_min_length(parser, static_cast<size_t>(magic_len)); // FIXME: BUG，size_t()，
        ...
        {
            size_t MAX_SIZE_AFTER_MAX_TOKENS_COUNT = 100000; // TODO： inputposskip_end_sizecache，pos + skip_end_size * MAX_SIZE_AFTER_MAX_TOKENS_COUNT
        ...
        ParserFunctor *ParserContext::generate_uvm_parser()
        {
            auto *ctx = this;
            auto l = [&](std::string t) {
            ...
            std::vector<uvm::parser::ParserFunctor*> expr1_stru = { ctx->lua_symbol_parser(), l("("), str_parser, l(")") };
            uvm::parser::ParserFunctor *expr_parser1 = ctx->concat_parsers(expr1_stru);
            ...
            */
            ...
            ctx->cache("args",
                ...
                n("tableconstructor"), ctx->string_parser(), n("exp"))); // n("exp")exp，exptablecontructor，2stat
                ...
                ->set_result_requirement([this](Input &input, MatchResult *mr)
            {
				...
				if (args_mr->node_name() == "prefixexp")
					return true;
                return false; // tableexp，parse2stat,
            })
				...
				, ctx->p_and(n("simple_type"), n("args"))->set_items_in_same_line()->set_result_requirement([this](Input &input, MatchResult *mr)
            {
				...
				return args_mr->head_token().token == "(" || (args_mr->node_name() == "tableconstructor" && args_mr->head_token().token == "{"); // TODO:  tableconstructor
            })
                ));
            ...
            ctx->cache("suffixedexp",
                ...
                // ctx->and(n("prefixexp"), n("args")) // FIXME: exp, suffixedexp, args，，
                ));
            ctx->cache("var",
                ctx->p_or(
            ...
            ctx->cache("stat",
                ...
                , n("exp") // FIXME: stat，，typecheckerparse
			...
			std::vector<size_t> tokens_positions)
		{
			...
			throw uvm::core::GluaException("Can't find item in cache");
		}
		...
		ParseResult UnionParserFunctor::tryParse(Input &input, size_t skip_end_size, bool optional)
		{
			...
			uvm::util::append_range(first_tokens, lookahead_tokens);

			if (first_token.token == "function")
				printf("");

			// FIXME: lookahead tokens
        ...
        ParseResult ParserContext::parse_syntax(Input &input)
        {
            ...
            if (!success)
            {
                // pr.stateSUCCESS，last_error_token，pr.stateSUCCESS,last_match_token
