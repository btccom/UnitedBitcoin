#include <uvm/ltype_checker_type_info.h>
	...
	namespace parser {
		...
		bool GluaTypeInfo::put_contract_storage_type_to_module_stream(GluaModuleByteStreamP stream)
		{
			if (!stream || !this->is_record())
				throw uvm::core::GluaException("empty stream or storage's type is not record");
			...
			for(const auto &p : this->record_props)
			{
				...
				{
					stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_int;
				}
				else if(prop_type->etype == GluaTypeInfoEnum::LTI_NUMBER)
				{
					stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_number;
				}
				else if(prop_type->is_bool())
				{
					stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_bool;
				}
				else if(prop_type->is_string())
				{
					stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_string;
				}
				else if (prop_type->is_stream_type)
				{
					stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_stream;
				}
				else if(prop_type->is_narrow_table())
				{
					...
					throw uvm::core::GluaException(error_str.c_str());
					// stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_unkown_table;
				}
				else if(prop_type->is_map())
				{
					...
					case GluaTypeInfoEnum::LTI_INT: {
						stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_int_table;
					} break;
					case GluaTypeInfoEnum::LTI_NUMBER: {
						stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_number_table;
					} break;
					case GluaTypeInfoEnum::LTI_BOOL: {
						stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_bool_table;
					} break;
					case GluaTypeInfoEnum::LTI_STRING: {
						stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_string_table;
					} break;
					case GluaTypeInfoEnum::LTI_STREAM: {
						stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_stream_table;
					} break;
					default: {
				...
				else if(prop_type->is_array())
				{
					...
					{
					case GluaTypeInfoEnum::LTI_INT: {
						stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_int_array;
					} break;
					case GluaTypeInfoEnum::LTI_NUMBER: {
						stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_number_array;
					} break;
					case GluaTypeInfoEnum::LTI_BOOL: {
						stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_bool_array;
					} break;
					case GluaTypeInfoEnum::LTI_STRING: {
						stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_string_array;
					} break;
					case GluaTypeInfoEnum::LTI_STREAM: {
						stream->contract_storage_properties[p.first] = uvm::blockchain::StorageValueTypes::storage_value_stream_array;
					} break;
					default: 
					{
