...
namespace fcrypto {
    ...
    sha256::sha256( const char *data, size_t size ) { 
	  ...
	  FJSON_THROW_EXCEPTION( exception, "sha256: size mismatch" );
    ...
    sha256 operator >> ( const sha256& h1, uint32_t i ) {
      sha256 result;
      fcrypto::detail::shift_r( h1.data(), result.data(), result.data_size(), i );
      return result;
    }
  ...
  void to_variant( const sha256& bi, fjson::variant& v )
  {
     v = std::vector<char>( (const char*)&bi, ((const char*)&bi) + sizeof(bi) );
  }
  void from_variant( const fjson::variant& v, sha256& bi )
  {
