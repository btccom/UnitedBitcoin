...
CAmount CWalletTx::GetImmatureCredit(bool fUseCache) const
{
	...
	else if (IsCoinStake() && GetBlocksToMaturity() > 0 && IsInMainChain())
    {
        if (fUseCache && fImmatureCreditCached)
            return nImmatureCreditCached;
        nImmatureCreditCached = pwallet->GetCredit(*tx, ISMINE_SPENDABLE);
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    ...
    if (gArgs.GetBoolArg("-walletrejectlongchains", DEFAULT_WALLET_REJECT_LONG_CHAINS)) {
        ...
        LockPoints lp;
        CTxMemPoolEntry entry(wtxNew.tx, 0, 0, 0, false, 0, lp, 0);
...
int CMerkleTx::GetBlocksToMaturity() const
{
   ...
   if (mi == mapBlockIndex.end()) {
   	...
   	if (!pindex || !chainActive.Contains(pindex)) {
   		...
   		coinBaseHeight = pindex->nHeight;
   	}
   }
   
   int numBlocksToMaturity = getCoinBaseMaturity(coinBaseHeight);
   
   //return std::max(0, (COINBASE_MATURITY+1) - GetDepthInMainChain());
   
   return std::max(0, (numBlocksToMaturity+1) - GetDepthInMainChain());
