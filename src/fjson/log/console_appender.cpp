...
namespace fjson {
   ...
   } FJSON_CAPTURE_AND_RETHROW( (console_appender_config) ) }
   ...
   void console_appender::log( const log_message& m ) {
      //fjson::string message = fjson::format_string( m.get_format(), m.get_data() );
      //fjson::variant lmsg(m);

      FILE* out = stream::std_error ? stderr : stdout;

      //fjson::string fmt_str = fjson::format_string( cfg.format, mutable_variant_object(m.get_context())( "message", message)  );
      std::stringstream file_line;
      file_line << "log" <<":"<<m.get_context().get_line_number() <<" ";
      ...
      fjson::unique_lock<boost::mutex> lock(log_mutex());
