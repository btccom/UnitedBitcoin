...
namespace fjson
{
   ...
   :my( std::make_shared<detail::log_context_impl>() )
   {
	  ...
	  my->thread_name = "thread";
	  const char* current_task_desc = "current_task_desc";
   ...
   fjson::string log_context::to_string()const
   {
      return my->thread_name + "  " + ":" + fjson::to_string(my->line) + " " + my->method;

   }

   void log_context::append_context( const fjson::string& s )
   {
        if (!my->context.empty())
          my->context += " -> ";
        my->context += s;
   ...
   void from_variant( const variant& v, log_level& e )
   {
      try 
      {
        ...
        else FJSON_THROW_EXCEPTION( bad_cast_exception, "Failed to cast from Variant to log_level" );
      } FJSON_RETHROW_EXCEPTIONS( error, 
